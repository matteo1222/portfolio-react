  ...
  return (
    ...
    >
      ...
      <div className="relative mt-12">
        ...
        <article className="my-16 flex md:justify-center md:items-center">
          <div
            ref={foodieClubRef}
            className={`relative w-full md:w-3/5 shadow-lg ${
              isFoodieClubVisible ? "md:animate-fadein-from-l" : "md:opacity-0"
            }`}
          >
            <ImgWithFallback
              src={foodieClubWebP}
            ...
            <div className="md:hidden flex flex-col items-start p-6 w-full h-full bg-slate-900 opacity-90">
              <h4 className="text-white text-xl font-semibold my-2">
                {textContent.projects.foodieClub.title}
              </h4>
              <h5 className="text-sky-500 text-left italic">
                {textContent.projects.foodieClub.subtitle}
              </h5>
              <p className="text-white text-left my-8">
                {textContent.projects.foodieClub.description}
              </p>
              {/* Tech Stack */}
              <div className="text-sky-500 flex flex-wrap justify-self-end text-xs">
                {textContent.projects.foodieClub.techStacks.map((tech, idx) => (
                  <span key={idx} className="mx-2 my-1">
                    {tech}
                  </span>
                ))}
              </div>
              ...
              <div className="flex items-center mt-6">
                <a
                  ...
                  href="https://github.com/matteo1222/foodie-club"
                  target="_blank"
                  rel="noreferrer"
                >
          ...
          <div className="hidden md:block w-96 p-6">
            <h4 className="text-slate-700 text-left text-2xl font-semibold my-2">
              {textContent.projects.foodieClub.title}
            </h4>
            <h5 className="text-sky-800 text-left italic">
              {textContent.projects.foodieClub.subtitle}
            </h5>
            <p className="text-slate-700 text-left my-8">
              {textContent.projects.foodieClub.description}
            </p>
            {/* Tech Stack */}
        ...
        <article className="my-16 md:mt-36 flex md:justify-center md:items-center">
          {/* The before part is the speaker of the smartphone */}
          <div
            ref={foodBagRef}
            className={`order-2 relative w-full shadow-lg md:w-80 md:h-auto md:aspect-[5/11] md:mr-32
          ...
          >
            ...
            <div className="md:hidden top-0 flex flex-col items-start p-6 w-full h-full bg-slate-900 opacity-90">
              <h4 className="text-white text-xl font-semibold my-2">
                {textContent.projects.foodBag.title}
              </h4>
              <h5 className="text-sky-500 text-left italic">
                {textContent.projects.foodBag.subtitle}
              </h5>
              <p className="text-white text-left my-8">
                {textContent.projects.foodBag.description}
              </p>
              {/* Tech Stack */}
              <div className="text-sky-500 flex flex-wrap justify-self-end text-xs">
                {textContent.projects.foodBag.techStacks.map((tech, idx) => (
                  <span key={idx} className="mx-2 my-1">
                    {tech}
                  </span>
                ))}
